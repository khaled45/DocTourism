...
router.post('/account', (req, res) => { // return data of doctor by ID
    ...
    doctorModel.findOne({
        _id: req.body.Did
    }).populate('patients').exec((err, doctor) => {
        if (err) {
            res.json({
                "message": "error"
            })
...
router.post('/createTreatmentPlan', verifyToken,(req, res) => { //need to handel req.sesion.user
    ...
    diagnosisModel.findOne({
        patientID: patientID,
        doctorID: doctorID
    }).exec((err, alreadySended) => {
        if (err) {
            res.json({
                "message": 'error'
            })
...
router.post('/addQuestions', (req, res) => { // need to handel req.sesion.user
    ...
    const {
        Questions,
        Did
    } = req.body // "Questions" is array of objects and each object contain all Questions like {"question" : "" , "type":""}
...
router.get('/getAllTreatment', verifyToken, (req, resp) => {
    DoctorId = req.userID
    treatmentPlanModel.find({
        doctorID: DoctorId
    }).populate('patientID').exec((err, data) => {
        err ? resp.json({
            "message": 'error'
        }) : resp.json({
            "message": 'success',
