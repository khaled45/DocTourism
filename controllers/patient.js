...
router.get('/account', verifyToken, (req, res) => {
    ...
    patientModel.findOne({ _id: req.userID }).populate('diagnosisForm').exec((err, patient) => {
        if (err) {
            res.json({ "message": "error" })
        }
        ...
        treatmentPlanModel.find({ patientID: req.userID }).populate('doctorID').exec((err, treatmentPlans) => {
            ...
            res.json({ "message": 'success', "data": treatmentPlans,"patient":patient })
        })
    });
});
        ...
        if (err) {
            ...
            if (accept_flag == "true") {
                ...
                treatment.save((err) => {
                    ...
                    res.json({ "message": "success",'data':treatment })
                })
